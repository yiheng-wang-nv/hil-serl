...
2. Inside 'labels', creates category folders: class_0, class_1, class_2, class_3, class_4, class_5
3. For each episode, copies frames to appropriate category folders based on their frame numbers
4. Frame categorization:
   - frame < pickup_left → class_0
   - pickup_left ≤ frame < pickup_right → class_1
   - pickup_right ≤ frame < align_central_aperture → class_2
   - align_central_aperture ≤ frame < assemble → class_3
   - assemble ≤ frame < placement → class_4
   - placement ≤ frame < end → class_5
"""
...
def get_category_for_frame(frame_num, episode_labels):
    ...
    if frame_num < episode_labels['pickup_left'] - 10:
        return 'class_0'
    elif frame_num < episode_labels['pickup_right'] - 10 and frame_num >= episode_labels['pickup_left']:
        return 'class_1'
    elif frame_num < episode_labels['align_central_aperture'] - 10 and frame_num >= episode_labels['pickup_right']:
        return 'class_2'
    elif frame_num < episode_labels['assemble'] - 10 and frame_num >= episode_labels['align_central_aperture']:
        return 'class_3'
    elif frame_num < episode_labels['placement'] - 10 and frame_num >= episode_labels['assemble']:
        return 'class_4'
    elif frame_num < episode_labels['end'] and frame_num >= episode_labels['placement']:
        return 'class_5'
    else:
        # Frames >= end are ignored or can be treated as class_0
        return None
